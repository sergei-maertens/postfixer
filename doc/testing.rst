...
1. ``postfixer-django``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``postfixer-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

**develop** branch
------------------

1. ``postfixer-django-develop``: backend tests, runs ``./bin/jenkins_django.sh``.
2. ``postfixer-django-develop-js``: frontend tests, runs ``./bin/jenkins_js.sh``.

pull requests
-------------
1. ``postfixer-pr-django``: backend tests, runs ``./bin/jenkins_django_pr.sh``.
2. ``postfixer-pr-js``: frontend tests, runs ``./bin/jenkins_js.sh``.
3. ``postfixer-pr-sass``: checks that sass compiles, runs ``./bin/jenkins_sass.sh``.
4. ``postfixer-pr-isort``: checks that imports are correctly
